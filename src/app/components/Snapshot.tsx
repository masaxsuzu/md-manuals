...
export const SnapShot = (props:{snapshot: Snapshot}) => {
  ...
  const field = props.snapshot.field ? toRow('banished', [props.snapshot.field]) : [];
  ...
  for (let index = 0; index < col*4; index++) {
    ...
    if([
        col*0, col*0 + 1, col*0 + 3,  col*0 + 5,
        col*3, col*3+(col-1)
      ].includes(index)) {
      tiles.push(<EmptyZone key={index} images={images} width={width} height={height} ></EmptyZone>)        
    }
    else if(index === col -1) {
      //banished
      tiles.push(<BanishedZone key={index} images={images} width={width} height={height} tiles={banished}></BanishedZone>)
    }
    else if(index === col) {
      //field
      tiles.push(<FieldZone key={index} images={images} width={width} height={height} tiles={field}></FieldZone>)
    }
    else if(index === 2*col - 1) {
      //graveyard
      tiles.push(<GraveyardZone key={index} images={images} width={width} height={height} tiles={graveyard}></GraveyardZone>)
    }
    else if(index === 2*col) {
      //extraDeck
      tiles.push(<ExtraDeckZone key={index} images={images} width={width} height={height} tiles={extraDeck}></ExtraDeckZone>)
    }
    else if(index === 3*col - 1) {
      //mainDeck
      tiles.push(<MainDeckZone key={index} images={images} width={width} height={height} tiles={mainDeck}></MainDeckZone>)
    }
    else {
      // other cards
      tiles.push(<CardZone key={index} images={images} width={width} height={height} item={item}></CardZone>);
    }
    if(index % col === (col-1)) {
