...
export function ns(cardId: number, zone: ZoneId, at: number): Action {
    return new NormalSummonAction(cardId, incrementOrder(), { zone: zone, at: at });
}
export function ss(cardId: number, zone: ZoneId, at: number): Action {
    return new SpecialSummonAction(cardId, incrementOrder(), { zone: zone, at: at });
}
...
export function toMainMonster(cardId: number, at?: number): Action {
    return new MoveAction(cardId, incrementOrder(), { zone: 'mainMonster', at: at });
}

export function toSpellAndTrap(cardId: number, at?: number): Action {
    return new MoveAction(cardId, incrementOrder(), { zone: 'spellAndTrap', at: at });
}

export function toGraveyard(cardId: number, at?: number): Action {
    return new MoveAction(cardId, incrementOrder(), { zone: 'graveyard', at: at });
}
...
class NormalSummonAction implements Action {
    constructor(public cardId: number, public order: number, public to: { zone: ZoneId, at?: number }) {
...
class XYZAction implements Action {
    constructor(public xyz: number, public order: number, public to: { zone: ZoneId, at: number }, public actions: Action[]) {
...
class LinkAction implements Action {
    constructor(public link: number, public to: { zone: ZoneId, at: number }, public actions: Action[]) {
...
class InspectAction implements Action {
    ...
    run(cards: CardStatus[]) {
        ...
        return `現在の状態:\n- 手札:${hand.map(x => x.name).join(',')}\n- 墓地:${graveyard.map(x => x.name).join(',')}\n- 除外ゾーン:${banished.map(x => x.name).join(',')}`
    }
}
