...
import { initCards, ef, ns, ss, xyz, link, inspect, toHand, toMainMonster, toGraveyard, toBanished, toSpellAndTrap, toXyz } from "../services/ActionService";
...
function _linkEx(cardId:number, at:number, m1:number,m2:number){
  return link(cardId, 'extraMonster', at, [
    toGraveyard(m1),
    toGraveyard(m2),
  ]);
}
...
function _xyzMain(cardId:number, at:number, m1:number,m2:number){
  return xyz(cardId, 'mainMonster', at, [
    toXyz(m1),
    toXyz(m2),
  ]);
}
...
function manual18() {
  ...
  const actions = [
    ...
    ef(ブレイクソード,[
      toMainMonster(スケイル,0),
      toMainMonster(ローブ2,1),
    ]),
...
function manual1a() {
  ...
  const actions = [
    ...
    ef(霧剣,[
      toBanished(霧剣),
      toMainMonster(スケイル,2),
    ]),
    _xyzMain(リヴァイ2,1,スケイル,ローブ),
    ef(リヴァイ2,[
      toGraveyard(スケイル),
      toMainMonster(グローブ,2),
    ]),
    _xyzMain(未来皇,0,リヴァイ,リヴァイ2),
    ef(未来龍皇,[
      toGraveyard(ローブ),
